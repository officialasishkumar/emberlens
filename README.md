...
emberlens issues -repo golang/go -view new -since 720h -period week
emberlens issues -repo golang/go -view backlog -stale-for 1440h
emberlens discover -repo golang/go -view untriaged -min-age 168h
```

To reveal more detail:

```bash
emberlens issues -repo golang/go -view resolution -verbose -limit 10
emberlens issues -repo golang/go -view participants -output json | jq
emberlens maintainers -repo keploy/keploy -signals -profiles -limit 0
emberlens discover -repo golang/go -view hotspots -since 336h -min-comments 5 -min-participants 3
```
...
```bash
emberlens issues -repo golang/go
emberlens issues -repo golang/go -view new -since 720h -period week
emberlens issues -repo golang/go -view active -since 720h -period day
emberlens issues -repo golang/go -view closed -since 720h -period week -unit days
emberlens issues -repo golang/go -view backlog -stale-for 720h -sort age
emberlens issues -repo golang/go -view age
emberlens issues -repo golang/go -view resolution -since 1440h -unit days -sort duration
emberlens issues -repo golang/go -view response -since 720h -comment-pages 2 -unit hours
emberlens issues -repo golang/go -view participants -since 720h -comment-pages 2
emberlens issues -repo golang/go -view abandoned -stale-for 720h -comment-pages 2
```
...
```bash
emberlens discover -repo golang/go -view untriaged -min-age 168h
emberlens discover -repo golang/go -view needs-maintainer -min-age 168h -min-comments 3 -min-participants 3
emberlens discover -repo golang/go -view hotspots -since 336h -min-comments 5 -min-participants 3
```
...
version: v2
name: test-run-0
command: "emberlens issues -repo golang/go -view=new -since=720h -period=week"
repo: golang/go
status: success
total: 5
...
Emberlens now covers people and issue analytics, but it still does not include major domains:
