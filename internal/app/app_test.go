...
func TestRunnerDiscoverUntriagedDispatchesCorrectly(t *testing.T) {
	now := time.Date(2026, 3, 16, 12, 0, 0, 0, time.UTC)
	client := &fakePlatformClient{
		issues: []platform.Issue{
			{
	...
	code := runner.Run([]string{"discover", "-repo", "owner/repo", "-view", "untriaged", "-output", "json", "-no-report"}, "", "")
	if code != 0 {
		t.Fatalf("Run() = %d, stderr=%s", code, stderr.String())
	}
	if len(client.issueCalls) != 1 {
	...
	if len(client.commentCalls) != 1 || client.commentCalls[0].number != 7 {
		t.Fatalf("commentCalls = %+v, want one call for issue 7", client.commentCalls)
	}
	if !strings.Contains(stdout.String(), `"title": "Untriaged issues"`) {
